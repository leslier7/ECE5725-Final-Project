...
def GPIO27_callback(channel):
    print("Exiting program from button press")
    global code_run
    code_run = False
...
actions = [(goStraight, 2), (stop, 1), (goBackwards, 2), (stop, 1), (turnLeft, 2), (stop, 1), (turnRight, 2), (stop, 1)]
...
try:
    ...
    current_action = 0 # index for the action array
    ...
    while ( code_run ):
        ...
        if (start == True and panic_value == False):
            elapsed_time = time.time() - action_timer
            #print(elapsed_time)
            if(int(elapsed_time) >= actions[j][1]):
                print("not panicing")
